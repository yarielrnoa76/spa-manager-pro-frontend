...
import type { AuthenticatedUser, SaleCreateContext, Tenant, TenantSalesMode } from '../../types';
import type { CreateSaleGroupResponse } from '../../types/payments';

/**
 * Capability-driven sale context microcorrection: CreateSaleModal now consumes
 * `useEffectiveSaleContext`, which is itself a thin client over `GET /api/sales/create-context`.
 * Every test here mocks `api.getSaleCreateContext` directly (never `listBranches` or
 * `listUserCandidates`, which this modal must never call at all) and asserts the component
 * renders/gates exactly what the backend response says -- regardless of the user's role name or
 * permission list, which are never read for these decisions.
 */
...
describe('CreateSaleModal — professionals/tenant-profile still gated by their own real permission', () => {
    it('never requests professionals or tenant/profile without the matching permission', async () => {
    ...
    it('requests them when the actor holds the matching permission', async () => {
        const user = baseUser({ id: '10', permissions: ['view_professionals', 'view_tenant_profile'] });
